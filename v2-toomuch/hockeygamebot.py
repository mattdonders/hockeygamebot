...
from handlers.game_status_checker import GameStatusChecker
from services.content_preparer import ContentPreparer
...
class HockeyGameBotApp:
    def __init__(self):
        ...
        # Populate All Teams Info
        self.teams_data = self.nhl_service.get_team_full_names()

        # Initialize variables
        self.team_name = self.args.team or self.config["default"]["team_name"]
        self.team_abbr = self.config["default"].get("team_abbr", "NJD")
        # TODO: Move hashtags into a generic function (or see if available via API)
        self.team_hashtag = self.config["default"].get("team_hashtag", self.team_abbr)
        self.team_id = self.teams_data.get(self.team_name, {}).get("team_id")
        self.team_info = {
            "name": self.team_name,
            "abbr": self.team_abbr,
            "hashtag": self.team_hashtag,
            "id": self.team_id,
        }
        logging.info("Full Team Info: %s", self.team_info)
        self.content_preparer = ContentPreparer(self.nhl_service, self.team_info)

        self.game_status_checker = GameStatusChecker(self.nhl_service)
        self.game_handler = GameHandler(self.nhl_service, self.content_preparer, self.social_media_service)

    def run(self):
        date = utils.ymd_date_parser(self.args.date) or datetime.now()

        # Check game status
        status, game_info = self.game_status_checker.check_game_status(self.team_abbr, date)
        logging.info(status, game_info)

        if status == "today":
            self.game_handler.handle_game_today(game_info)
        elif status == "yesterday":
            self.game_handler.handle_game_yesterday(game_info)
        else:
            logging.info("No game scheduled for today or yesterday. Exiting.")
